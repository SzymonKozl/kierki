...
std::pair<Side, int> GameRules::whoTakes(
        const Side &startingSide,
        const Table &table,
        RoundType roundType,
        size_t trickNo
        ) {
    ...
    Color wanted = table.front().getColor();
    for (int i = 1; i < 4; i ++) {
        if (table[i].getColor() == wanted && table[i] > table[takerIx])
            takerIx = i;
    }
    Side takerSide = startingSide;
    ...
    switch (roundType) {
        ...
        case HEART_PENALTY: {
            pen = 0;
            for (const Card& c: table) if (c.getColor() == COLOR_H)
                pen += HEART_PENALTY_VAL;
            return {takerSide, pen};
        }
        case QUEEN_PENALTY: {
            pen = 0;
            for (const Card& c: table) if (c.getValue() == "Q")
                pen += QUEEN_PENALTY_VAL;
            return {takerSide, pen};
        }
        case KING_JACK_PENALTY: {
            pen = 0;
            for (const Card& c: table)
                if (c.getValue() == "K" || c.getValue() == "J")
                    pen += KING_JACK_PENALTY_VAL;
            return {takerSide, pen};
        }
        case HEART_KING_PENALTY: {
            pen = 0;
            for (const Card& c: table)
                if (c.getValue() == "K" && c.getColor() == COLOR_H)
                    pen += KING_HEART_PENALTY_VAL;
            return {takerSide, pen};
        }
        case SEVENTH_LAST_TRICK_PENALTY: {
            return {
                takerSide,
                (trickNo == 7 || trickNo == TRICKS_PER_ROUND) ?
                    SEVENTH_LAST_TRICK_PENALTY_VAL : 0
            };
        }
        case EVERYTHING: {
            pen = TRICK_PENALTY_VAL;
            pen += (trickNo == 7 || trickNo == TRICKS_PER_ROUND) ?
                    SEVENTH_LAST_TRICK_PENALTY_VAL : 0;
            for (const Card& c: table) {
